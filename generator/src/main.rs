...
use std::old_io::{File, BufferedReader, BufferedWriter, self};
use std::iter::repeat;
...
fn main() {
    ...
    let mut ctxt = Context {
        out: match file {
            Some(ref p) => Box::new(BufferedWriter::new(File::create(&p.with_extension("tmp"))))
                as Box<Writer>,
            None => Box::new(old_io::util::NullWriter) as Box<Writer>
        }
    };
    ...
    match file {
        Some(f) => old_io::fs::rename(&f.with_extension("tmp"), &f).unwrap(),
        None => {}
    }
}
